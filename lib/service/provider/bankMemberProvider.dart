...
class BankMemberProvider with ChangeNotifier {
  ...
  store({BuildContext context, Map<dynamic, dynamic> data}) async {
    ...
    final isValid = valid.validateEmptyForm(
        context: context,
        field: {"atas_nama": data["acc_name"], "no_rekening": data["acc_no"]});
    if (isValid) {
      dynamic res;
      if (isAdd) {
        data.remove("id");
        res = await HttpService()
            .post(url: "bank_member", data: data, context: context);
      } else {
        ...
        print(data);
        res = await HttpService()
            .put(url: "bank_member/$id", data: data, context: context);
        print(res);
      }
      if (res != null) {
        get(context: context);
    ...
    FunctionalWidget.nofitDialog(
        ...
        callback2: () async {
          ...
          final res = await HttpService().delete(
              url: "bank_member/${bankMemberModel.result[indexBank].id}",
              context: context);
          if (res != null) {
            Navigator.of(context).pop();
            get(context: context);
