...
class UserProvider with ChangeNotifier {
  ...
  bool isLoadingLeaderBoard = true,
      isLoadingDetailMember = true,
      isLoadingListReferral = true,
  ...
  getDetailMember({BuildContext context}) async {
    ...
    final res =
        await HttpService().get(url: "member/get/$idUser", context: context);
  ...
  getLeaderBoard({BuildContext context}) async {
    ...
    final res = await HttpService().get(
        url: "member/top_kontributor?page=1&type=penjualan", context: context);
  ...
  store({BuildContext context, fields, isPin = false}) async {
    final res = await HttpService()
        .put(url: "member/$idUser", data: fields, context: context);
    if (res != null) {
      if (isPin) {
  ...
  Future getListReferral({BuildContext context}) async {
    ...
    final res = await HttpService().get(
        url: "member/referral?page=1&perpage=$perPageListReferral",
        context: context);
    if (res["result"].length > 0) {
