  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: FunctionalWidget.appBarWithFilterHelper(
          ...
          action: [
            if (!isLoading &&
                member.idUser == product.detailProductModel.result.idSeller)
              FunctionalWidget.iconAppbar(
                  context: context,
                  callback: () {
                    product.setDataEditProductContributor(
                        product.detailProductModel.result.toJson());
                    product.setIsAdd(false);
                    product.setStatusProduct(
                        product.detailProductModel.result.status);
                    Navigator.of(context)
                        .pushNamed(RouteString.formProductContributor);
                  },
                  image: "Edit",
                  color: general.conditionFilterProductContributor
            ...
            children: [
              isLoading
                  ...
                  : Container(
                      ...
                      child: Text(product.detailProductModel.result.title,
              ...
              isLoading
                  ...
                  : InkResponse(
                      ...
                      child: Icon(AntDesign.heart,
                          color: favorite.detail.length > 0 &&
                                  favorite.detail[0]["checked"] == "1"
                              ? Colors.red
                              : Colors.black),
                    )
            ],
          ...
          Row(
            children: [
              isLoading
                  ...
                  : FunctionalWidget.rating(
                      context: context,
                      rate: double.parse(
                          product.detailProductModel.result.rating.toString())),
              ...
              isLoading
                  ? BaseLoading(height: 1, width: 10)
                  : Text(product.detailProductModel.result.terjual,
          ...
          FunctionalWidget.wrapContent(
              child: InTouchWidget(
                  callback: () => Navigator.of(context).pushNamed(
                      ...
                      arguments: product.detailProductModel.result.idSeller),
                  child: ListTile(
                    leading: isLoading
                        ? BaseLoading(height: 4, width: 10, radius: 100)
                        : CircleAvatar(
                            backgroundColor: Colors.transparent,
                            radius: 20,
                            backgroundImage: NetworkImage(
                                product.detailProductModel.result.sellerFoto)),
                    title: isLoading
                        ? BaseLoading(height: 1, width: 10)
                        : Text(
                            product.detailProductModel.result.seller,
                            style: Theme.of(context).textTheme.headline2,
                          ),
                    subtitle: isLoading
                        ? BaseLoading(height: 1, width: 1)
                        : Text(
                            product.detailProductModel.result.sellerBio == null
                                ? "-"
                                : product.detailProductModel.result.sellerBio,
                            style: Theme.of(context).textTheme.subtitle1,
                            maxLines: 1,
                            overflow: TextOverflow.ellipsis,
                          ),
                    trailing: isLoading
                        ? BaseLoading(height: 4, width: 10, radius: 10)
                        : Container(
          ...
          if (!isLoading &&
            ...
            Column(
              ...
              children: [
                ...
                Text(
                  product.detailProductModel.result.preview,
                  style: Theme.of(context).textTheme.headline2,
                ),
                SizedBox(height: scale.getHeight(1)),
                Text("Konten", style: Theme.of(context).textTheme.headline1),
                Text(
                  product.detailProductModel.result.content,
                  style: Theme.of(context).textTheme.headline2,
                ),
              ],
            )
            ...
            Column(
              ...
              children: [
                Text(
                    product.detailProductModel.result.statusBeli == 1
                        ? "Konten"
                        : "Preview",
                    style: Theme.of(context).textTheme.headline1),
                Text(
      ...
      bottomNavigationBar: FunctionalWidget.bottomBar(
          ...
          desc: isLoading
              ? "loading ......"
              : FunctionalWidget.toCoin(
                  double.parse(product.detailProductModel.result.price)),
          btnText: isLoading
              ? "loading ......"
              : product.detailProductModel.result.statusBeli == 1
                  ? "Ambil tulisan"
          ...
          callback: () async {
            if (!isLoading &&
                product.detailProductModel.result.statusBeli == 1) {
              await Share.share(
                  removeAllHtmlTags(product.detailProductModel.result.content));
    ...
    return Column(
      ...
      children: [
          ...
          callback: () async {
            ...
            if (res == "false") {
              FunctionalWidget.nofitDialog(
                  ...
                  callback2: () => Navigator.of(context).pop());
            }
        ...
        CardAction(
          title: "Twitter",
          callback: () async {
            final res = await flutterShareMe.shareToTwitter(
                url: obj["url"], msg: obj["msg"]);
            if (res == "false") {
              FunctionalWidget.nofitDialog(
                  context: context,
                  msg: "aplikasi tidak ditemukan",
          ...
          img: "",
          imgNetwork:
              "https://4.bp.blogspot.com/-WYBZhMb9BMw/V-s6c2P9uXI/AAAAAAAABN8/dRa9fkfvz6A-nf3i2QAZIm8X87O5ja9GQCEw/s1600/logo-twitter-t-bulat-348x348.png",
        ),
        CardAction(
          title: "Clipboard",
          callback: () async {
