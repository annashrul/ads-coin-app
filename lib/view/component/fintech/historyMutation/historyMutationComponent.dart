...
class HistoryMutationComponent extends StatefulWidget {
  @override
  _HistoryMutationComponentState createState() =>
      _HistoryMutationComponentState();
}
...
class _HistoryMutationComponentState extends State<HistoryMutationComponent> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: FunctionalWidget.appBarHelper(context: context, title: "Mutasi"),
      body: Column(
        children: [
          FunctionalWidget.filterDate(
              context: context,
              data: {
                "from": history.fromHistoryMutation,
                "to": history.toHistoryMutation
              },
              callback: (from, to) {
                history.setDateFromDateToHistoryMutation(
                    context: context, input: {"from": from, "to": to});
              }),
          Expanded(
            child: history.isLoadingHistoryMutation
                ? LoadingHistoryPurchase()
                : history.historyMutationModel == null
                    ? NoDataWidget()
                    : ListView.separated(
                        ...
                        itemBuilder: (context, index) {
                          final val =
                              history.historyMutationModel.result[index];
                          String nominal =
                              "+ ${FunctionalWidget.toCoin(double.parse(val.trxIn))}";
                          Color colorNominal = ColorConfig.blackPrimaryColor;
                          if (double.parse(val.trxOut) > 0) {
                            nominal =
                                "- ${FunctionalWidget.toCoin(double.parse(val.trxOut))}";
                            colorNominal = ColorConfig.redColor;
                          }
                          Color color;
                          if (val.type == "Penarikan")
      ...
      bottomNavigationBar: history.isLoadMoreHistoryMutation
          ? Container(
