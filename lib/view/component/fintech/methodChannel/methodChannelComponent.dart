  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body:  ListView.separated(
        ...
        itemCount: channel.isLoading?2:channel.channelPaymentModel.result.length,
        itemBuilder: (context,index){
          return channel.isLoading?Container(
            child: ListTile(
              leading: BaseLoading(height: 3,width: 8,radius: 100),
              title: Container(
                child: BaseLoading(height: 1,width: MediaQuery.of(context).size.width/3),
              ),
            ...
            callback: (){
              widget.callback(channel.channelPaymentModel.result[index].code);
            },
          );
        },
      )
